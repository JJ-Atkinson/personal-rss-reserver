...
;; SEE deps.edn, I've added a jdk override to allow host to be set. This is in response to a signing error, where host 
;; was set to `garage-ct.lan` by cognitect but java..httprequest tried to set it to `garage-ct.lan3900`, which is obviously 
;; not the same and breaks the signatures.
(def restricted-headers #{#_"host"})
