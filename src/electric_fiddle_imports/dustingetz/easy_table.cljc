  ...
  (e/client
   (dom/div (dom/props {:class "Viewport"})
     (let [row-height     24 ; todo parameterize
       ...
       (e/amb ; client
        (dom/table (e/server (dom/props {:style {:--row-height (str row-height "px")
                                                 :top          (str (* offset row-height) "px")}}))
          (let [selected (dom/On "focusin"
                                 (fn [event]
                                   (-> event
                                       .-target
                                       (aget "tablescroll-row-id")))
                                 selected)]
            (e/server
...
(e/defn Load-css
  [resource-path]
  (dom/style (dom/text (e/server (some-> (clojure.java.io/resource resource-path)
                                         slurp-safe)))))
...
(e/defn DemoEasyTable
  []
  (e/client (dom/props {:class "DemoEasyTable"})
