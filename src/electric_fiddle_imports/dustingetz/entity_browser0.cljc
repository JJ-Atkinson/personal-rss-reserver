...
(e/defn TreeRow
  ...
  (let [name (e/server (peek path))]
    (e/client
     (dom/td (dom/props {:style {:padding-left (e/server (some-> path
                                                                 count
                                                                 dec
                                                                 (* 15)
                                                                 (str "px")))}})
  ...
  (e/client
   (dom/fieldset (dom/props {:class "entity"})
                 ...
                 (let [xs!        (e/server (ex/Offload-reset #(drop 1 (explorer-seq m))))
                       ...
                       row-height 24
                       selected-x (e/server (first (filter (fn [[path _ _]] (= p-next path)) xs!)))] ; slow, but the
                                                                                                     ; documents are
                                                                                                     ; small
                   (dom/props {:style {:--col-count 2 :--row-height row-height}})
                   (Intercept (e/fn [index]
                                (TablePicker! ::select
                                              index
                                              row-count
                                              (e/fn [index]
                                                (e/server (some-> (nth xs! index nil)
                                                                  TreeRow)))
                                              :row-height
                                              row-height))
                              selected-x
   ...
   (dom/fieldset (dom/props {:class "entity-children"})
                 (let [xs!        (e/server (vec xs!))
                       ...
                       cols       (dom/legend (dom/text title)
                                              (ColumnPicker (e/server (ex/Offload-reset #(-> xs!
                                                                                             first
                                                                                             datafy
                                                                                             keys
                                                                                             #_sort
...
(e/defn Block
  [locus x]
  (e/client
   (when-some [F (e/server (case (infer-block-type x)
                             :tree   TreeBlock
                             :table  TableBlock
                             :scalar nil
                             nil))]
     (Interpreter {::select (e/fn [path]
                              (router/Navigate! ['. (if path [path] [])])
