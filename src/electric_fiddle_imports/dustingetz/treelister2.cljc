...
(defn records-coll?
  [?coll]
  (and (sequential? ?coll) (map? (first ?coll))))
...
(defn node-val
  [render-value node]
  (render-value (zip/node node)))

(defn tree-seq2
  [tree branch? children render-value]
  (->> tree
       (zip/zipper branch? children (fn make-node [& _] (throw (ex-info "Edits not implemented" {}))))
       (iterate zip/next) ; like `cc/tree-seq`, but remember path to nodes. Depth-first, in seq order, not
                          ; recursive but still heap bound.
       (sequence ; fully lazy
        (comp (take-while (complement zip/end?)) ; stop at end of walk, if there is such end and it has been
                                                 ; reached.
              #_(remove (fn [x]
                          (some-> x
                                  zip/up
                                  zip/node
                                  children
                                  empty?))) ; SLOW, is this accidentally polynomial?
...
(defn explorer-seq
  ...
  ;; - can lazily traverse deep nested trees (or cyclic graphs) - impl doesn't use recursion, but still heap-bound
  ;; as we accumulate `path`.
  ...
  (letfn [(branch? [x]
          ...
          (children [x]
            (cond (map? x)                      (seq x)
                  ...
                  (or (sequential? x) (set? x)) (map-indexed map-entry x))) ; simple collections treated as indexed
                                                                            ; maps
          (render-value [value]
...
(comment
  (explorer-seq nil)
  :=
  [[[] nil false]] ; emit root of tree - caller can drop
  (explorer-seq {})
  :=
  [[[] {} false]]
  (explorer-seq [])
