(ns personal-rss-feed.admin.electric-jetty-adapter
  "Slightly modified version of `electric-starter-app.server-jetty`
...
with target specified by map `m`. Target values are coerced to string with `str`.
  E.g. (template \"<div>$:foo$</div>\" {:foo 1}) => \"<div>1</div>\" - 1 is coerced to string."
    [t m]
    (reduce-kv (fn [acc k v] (str/replace acc (str "$" k "$") (str v))) t m))

;;; Template and serve index.html



;; This is now moved to personal-rss-feed.admin.pages.electric-index
#_
...
(defn http-middleware
  ...
  (-> nil
      ...
      ;;  (moved to personal-rss-feed.admin.routes)
...
(defn- add-gzip-handler!
  ...
  (.setHandler server
               (doto (GzipHandler.)
                 #_(.setIncludedMimeTypes (into-array ["text/css" "text/plain" "text/javascript"
                                                       "application/javascript" "application/json" "image/svg+xml"])) ; only
                                                                                                                      ; compress
                                                                                                                      ; these
                 (.setMinGzipSize 1024)
