      ...
      (if (:episode/audio-content-length episode)
        ...
        (do
          ...
          (file-utils/with-tempfile
           ...
           (throwing-shell "ffmpeg"
                           "-i"
                           (.toString video-temp)
                           "-q:a"
                           "0" ;; variable bitrate
                           "-map"
                           "a" ;; Extract audio only, no subtitles
           ...
           (db/save-episode! conn
                             (assoc episode
                                    ...
                                    (s3/content-length! s3 s3-audio-key))))
...
(comment
  ...
  (s3/download-object! (::le.shared/s3 @le.shared/!shared)
                       "audio-a466e883-b8c1-422f-9a01-f2a566738dfd.mp3"
                       (.toString dest))
  ...
  (extract-audio @le.shared/!shared
                 {::queue-item/data
                  {:episode/url
                   "https://www.lotuseaters.com/premium-live-lads-hour-10-or-birthday-special-09-11-2023"}})
