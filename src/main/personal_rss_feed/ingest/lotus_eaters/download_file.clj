    ...
    (let [episode        (db/episode-by-url (d/db conn) url)
      ...
      (when (= ::video download-type)
        (simple-queue/qsubmit! queue
                               {::queue-item/queue    ::le.extract-audio/extract-audio-queue
...
(comment
  ...
  (doseq [{::queue-item/keys [id data]} (simple-queue/qview
    ...
    (let [data (db/episode-by-url (d/db (:db/conn @le.shared/!shared)) (:episode/url data))]
      (simple-queue/update!qi
       ...
       id
       ::queue-item/priority
       (constantly (.getTime (:episode/publish-date data))))
  ...
  (d/touch (db/episode-by-url (d/db (:db/conn @le.shared/!shared))
                              "https://www.lotuseaters.com/premium-why-ideology-is-theology-14-07-23"))

  (db/save-episode! (:db/conn @le.shared/!shared)
                    {:episode/url
