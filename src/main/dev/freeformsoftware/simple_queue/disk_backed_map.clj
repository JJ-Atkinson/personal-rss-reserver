...
(pk/def-map-type DiskBackedMap
                 [!keys-cache !vals-cache k-pred read-fn write-fn]
...
(defn create-disk-backed-map!
  ...
  (let [read-fn             (or read-fn
                                (fn [k _] (->> k (key->file-name) (io/file folder-str) (slurp) (edn/read-string))))
