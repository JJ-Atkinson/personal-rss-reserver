...
(defn jwt-claims
  ...
  (let [claims (jwt/unsign message (:secret jwt-options) (:buddy jwt-options))]
    (when-not (:exp claims)
      (throw (ex-info "The EXP claim is missing and could not be verified!" {:claims claims})))
    claims))
...
(defn generate-jwt-from-credentials
  ...
  (enc/when-let [user (d/entity (d/db conn) [:user/uname username])
                 {:keys [valid]} (hashers/verify password-cleartext (:user/password-crypt user))]
    (if valid 
      (generate-jwt config (select-keys user [:user/uname :user/admin?]))
      (throw (ex-info "Credentials invalid!" {:username username :password password-cleartext})))))
...
(defmethod ig/suspend-key! ::auth
  [_ _])

(defmethod ig/resume-key ::auth
  [_ _ _ _])

(defmethod ig/halt-key! ::auth
  [_ conn]
  nil)
